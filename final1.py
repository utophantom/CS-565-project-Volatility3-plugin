...
### MODIFY THIS TO CUSTOMIZE PROCESS NAME FILTER ###

# Each process name can be added as a new item in this list
edit_this_list = ["cmd.exe", "powershell.exe"]


# Either set True or False
# True: List all processes, excluding the process names in edit_this_list
# False: Only list processes found in memory dump that matches edit_this_list items 
edit_this_exclude = False 

### END OF MODIFY ###

class CheckSusProcess(interfaces.plugins.PluginInterface, timeliner.TimeLinerInterface):
    ...
    def _generator(self):
        ...
        for proc in self.list_processes(
            self.context,
            self.config["kernel"],
            filter_func=self.create_name_filter(name_list=edit_this_list, exclude=edit_this_exclude),
        ):
